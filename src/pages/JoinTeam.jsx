...
const JoinTeam = () => {
  ...
  if (!e.target.phone.value.trim()) newErrors.phone = "Моля, въведете телефон";
  if (!e.target.privacy.checked) setPrivacyError(true);

  if (Object.keys(newErrors).length > 0 || !e.target.privacy.checked) {
    setErrors(newErrors);
    setTimeout(() => { setErrors({}); setPrivacyError(false); }, 3000);
    return;
  }
    setStatus("sending");
    ...
    <div className="  overflow-hidden">
      ...
      <section className="relative py-20 md:py-32 px-4 md:px-6 bg-white overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto relative z-10">
          ...
          {/* 1. ЗАГЛАВНА ЧАСТ */}
          <div className=" text-center max-w-7xl mx-auto pt-10 md:pt-20 px-6">
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
            ...
            >
              <h3 className="text-4xl md:text-6xl font-display font-medium text-brand-dark tracking-tighter leading-none mb-8">
                Стабилност <br />
                <span className="text-brand-primary font-light italic">
              ...
              <div className="border-y border-brand-light relative">
                ...
                <h3 className="font-display text-2xl md:text-3xl text-brand-primary italic pl-10 leading-relaxed">
                  "Партньорството с Forever ви дава не само финансова свобода, но
                  и спокойствието на една етична и дългосрочна основа."
                </h3>
              </div>
            </motion.div>
          </div>
          ...
          <div className="pt-10 md:pt-20 px-6 grid grid-cols-1 md:grid-cols-3 gap-8">
            {[
              {
                label: "Сигурност",
  ...
  <form onSubmit={handleSubmit} noValidate className="space-y-8"> {/* noValidate спира системните съобщения */}
    <div className="grid md:grid-cols-2 gap-8 text-left">
      
      {/* Поле Име */}
      <div className="space-y-3">
        <label className={`font-sans font-regular text-brand-dark block transition-colors ${errors.name ? "text-red-500" : ""}`}>
          Твоето име
        </label>
        <input
          name="name"
          ...
          className={`w-full bg-brand-cream border-2 px-6 py-4 rounded-2xl outline-none transition-all font-sans text-sm text-brand-dark ${
      ...
      <div className="space-y-3 text-left">
        <label className={`font-sans font-regular text-brand-dark block transition-colors ${errors.phone ? "text-red-500" : ""}`}>
          Телефонен номер
        </label>
        <input
          name="phone"
  ...
  <div className={`flex items-start gap-3 py-2 text-left transition-all duration-300`}>
    <div className="relative flex items-center h-5">
      <input
        id="privacy"
        name="privacy" // ВАЖНО: името съвпада с проверката в handleSubmit
        type="checkbox"
    ...
    <label htmlFor="privacy" className={`text-xs md:text-sm font-sans leading-tight cursor-pointer transition-colors ${privacyError ? "text-red-500" : "text-gray-500"}`}>
      Съгласен съм с{" "}
